...
export function SettingsRail({
  ...
  return (
    <nav
      ...
      className={cn(
        'flex gap-1 overflow-x-auto pb-2 snap-x snap-mandatory scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden',
    ...
    >
      {RAIL_GROUPS.map(({ label, group }) => {
        const items = SETTINGS_SECTIONS.filter((s) => {
          ...
          if (meta.superAdminOnly && !isSuperAdmin) return false;
          return true;
        });
            ...
            {items.map((s) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'flex shrink-0 snap-center items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm font-medium whitespace-nowrap transition-colors',
                    'lg:w-full',
                    isActive
