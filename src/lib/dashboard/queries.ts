...
export async function loadMetrics(db: DB): Promise<MetricsBundle> {
  ...
  const [
    ...
    db
      ...
      .gte('created_at', yesterdayStart)
      .lt('created_at', todayStart),
    db.from('deals').select('value, status'),
    db
      .from('messages')
      .select('id', { count: 'exact', head: true })
  ...
  const allDeals = (allDealsRes.data ?? []) as { value: number | null; status: string }[]
...
export async function loadPipelineDonut(db: DB): Promise<PipelineDonutData> {
  const [stagesRes, dealsRes] = await Promise.all([
    ...
    db.from('deals').select('stage_id, value, status'),
  ])

  const stages =
    (stagesRes.data ?? []) as { id: string; name: string; color: string }[]
  const deals = (dealsRes.data ?? []) as { stage_id: string; value: number | null; status: string }[]
