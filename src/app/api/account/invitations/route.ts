...
function getBaseUrl(request: Request): string {
  ...
  if (explicit && !explicit.includes("wacrm")) return explicit.replace(/\/+$/, "");
  ...
  const forwardedProto = request.headers
    ...
    ?.split(",")[0]
    ?.trim();
  if (forwardedHost && !forwardedHost.includes("wacrm") && isHostAllowed(forwardedHost, allowList)) {
    return `${forwardedProto || "https"}://${forwardedHost}`;
  }

  const host = request.headers.get("host")?.trim();
  if (host && !host.includes("wacrm") && isHostAllowed(host, allowList)) {
  ...
  return "https://vorder-app.vercel.app";
}
