...
export default function AutomationsPage() {
  ...
  async function toggleActive(a: Automation, next: boolean) {
    ...
    if (!res.ok) {
      ...
      toast.error(body?.error ?? tToasts("updateError"))
      return
    }
    toast.success(next ? tToasts("activated") : tToasts("paused"))
  }
  ...
  async function duplicate(a: Automation) {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? tToasts("duplicateError"))
      return
    }
    toast.success(tToasts("duplicated"))
    load()
  }
  ...
  async function confirmDelete() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? tToasts("deleteError"))
      return
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{tList("title")}</h1>
      ...
      {showTemplates && (
        <section>
          <h2 className="mb-3 text-sm font-semibold text-muted-foreground">{tList("templatesTitle")}</h2>
      ...
      {automations.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card/40">
          ...
          <p className="mt-3 text-sm font-medium text-foreground">{tList("emptyTitle")}</p>
